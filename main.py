...
def main():
    ...
    # добавляем старый wake_word для обратной совместимости
    ...
    logger.info("🤖 Jarvis запущен и готов к работе!")
    ...
    while True:
        try:
            ...
            logger.info(f"🧠 Распознано ({lang}): {text}")
            ...
            if not active_mode:
                ...
                if triggered_word:
                    ...
                    logger.info(f"🚀 Активирован wake word: '{triggered_word}'")
                    tts_queue.put(("Слушаю вас.", lang))

                    # Обрабатываем команду сразу, если есть после wake-word
                    cleaned = clean_text(normalized, triggered_word)
                    if cleaned:
